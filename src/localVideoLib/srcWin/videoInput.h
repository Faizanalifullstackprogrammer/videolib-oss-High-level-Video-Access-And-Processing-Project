...
//VI_MAX_RESOLUTIONS is hardcoded for now, but it would be nice
//to make deviceResolutions dynamically sized, depending on the
//actual number of resolutions supported by the device, vs the
//hardcoded value.
#define VI_MAX_RESOLUTIONS  60
...
class videoInput{
	...
	public:
		...
		//...if logFn is NULL and not silent, will output to stderr
		static int listDevices(bool silent = false, log_fn_t logFn = NULL);
		static int listDevicesWithoutResolutionList(bool silent = false, log_fn_t logFn = NULL);

		//needs to be called after listDevices - otherwise returns NULL
		static char * getDeviceName(int deviceID);

		//gets the number of supported resolutions from |deviceID|, and stores it in |numSupportedResolutions|.
		//Returns HRESULT or COM error codes.
		static int getNumSupportedResolutionsOfDevice(int deviceID, int *numSupportedResolutions, log_fn_t logFn = NULL);

		//gets the resolution height and width from |deviceID|, indexed by |resPairIdx|, and stores it in |width| and |height|.
		//Returns HRESULT or COM error codes.
		static int getSupportedResolutionPairOfDevice(int deviceID, int resPairIdx, int *width, int *height, log_fn_t logFn = NULL);
	...
	private:
        ...
        static int _listDevices(bool silent, log_fn_t logFn, bool cacheResolutionList);
		static int cacheDeviceSupportedResolutions(int deviceID, VARIANT *varName, IMoniker *pMoniker, log_fn_t logFn);
		...
		//don't touch
		static bool comInit(log_fn_t logFn);
        ...
        static char numResolutionsPerDevice[VI_MAX_CAMERAS];
        // The third dimension stores width and height in positions 0 and 1, respectively.
        static int deviceResolutions[VI_MAX_CAMERAS][VI_MAX_RESOLUTIONS][2];
