 ...
 *  Substantial portion of this module was derived or copied from https://github.com/ofTheo/videoInput
 *
 *****************************************************************************
 *
...
#include "fourcc_patch.h"

//for threading
#include <process.h>


// The safest way to call the Realease method is to use the macro SAFE_RELEASE that
// can be found in dxutil.h located in C:\DXSDK\samples\Multimedia\Common\include.
// It is simply: "#define SAFE_RELEASE(p) { if(p){(p)->Release();(p)=NULL;}}"
#define SAFE_RELEASE(p) { if(p){(p)->Release();(p)=NULL;}}
#define SAFE_RELEASE_AM_MEDIA_TYPE(p) { if(p){MyDeleteMediaType(p);(p)=NULL;}}
...
// Helper function to associate a name with the connection type
...
// Display I/O and routing information for video encoders
void DisplayCrossbarInfo(log_fn_t logFn, IAMCrossbar *pXBar)
{
    HRESULT hr;
    ...
    for (i = 0; i < cOutput; i++)
    {
        ...
        log_info(logFn, "  Output pin %ld: %s", i, GetPhysicalPinName(lType));
        log_info(logFn, "\tRelated out: %ld, Routed in: %ld", lRelated, lRouted);
        log_info(logFn, "\tSwitching Matrix: ");

        for (long j = 0; j < cInput; j++)
        {
            hr = pXBar->CanRoute(i, j);
            log_info(logFn, "\t\t%ld-%s", j, (S_OK == hr ? "Yes" : "No"));
        }
        log_info(logFn, "\n");
    }
    ...
    for (i = 0; i < cInput; i++)
    {
        ...
        log_info(logFn, "  Input pin %ld - %s", i, GetPhysicalPinName(lType));
        log_info(logFn, "\tRelated in: %ld", lRelated);
    }
    log_info(logFn, "\n");
}
...
public:
	...
	//------------------------------------------------
	SampleGrabberCallback(log_fn_t _logFn){
		logFn = _logFn;
    	...
    	if(hr == S_OK){
	    	latestBufferLength = pSample->GetActualDataLength();
	      	if(latestBufferLength == numBytes){
				...
				log_err(logFn, "ERROR: SampleCB() - buffer sizes do not match\n");
			}
		}
...
videoDevice::videoDevice(log_fn_t _logFn){
		 logFn = _logFn;
		 ...
		 sgCallback			= new SampleGrabberCallback(logFn);
...
void videoDevice::setSize(int w, int h){
	if(sizeSet){
		log_warn(logFn, "SETUP: Error device size should not be set more than once \n");
	}
	else
	{
		width 				= w;
...
void videoDevice::destroyGraph(){
	...
	while (hr == NOERROR)
	{
		...
		if (FAILED(hr)) { log_warn(logFn, "SETUP: pGraph->EnumFilters() failed. \n"); return; }
		...
		if (pEnum->Next(1, &pFilter, &cFetched) == S_OK)
		{
			...
			if(verbose)log_info(logFn, "SETUP: removing filter %s...\n", buffer);
			hr = pGraph->RemoveFilter(pFilter);
			if (FAILED(hr)) { log_warn(logFn, "SETUP: pGraph->RemoveFilter() failed. \n"); return; }
			if(verbose)log_info(logFn, "SETUP: filter removed %s  \n",buffer);
...
videoDevice::~videoDevice(){
	...
	if(setupStarted){ if(verbose)log_info(logFn, "\nSETUP: Disconnecting device %i\n", myID); }
	else{
		if(sgCallback){
			sgCallback->Release();
			delete sgCallback;
    ...
    if( (sgCallback) && (pGrabber) )
    {
        ...
        if(verbose)log_info(logFn, "SETUP: freeing Grabber Callback\n");
        sgCallback->Release();
	...
	{
		HR = pControl->Pause();
		if (FAILED(HR)) log_warn(logFn, "ERROR - Could not pause pControl\n");

		HR = pControl->Stop();
		if (FAILED(HR)) log_warn(logFn, "ERROR - Could not stop pControl\n");
    }
	...
	if( (pDestFilter) ){ 		if(verbose)log_info(logFn, "SETUP: freeing Renderer \n");
								(pDestFilter)->Release();
								(pDestFilter) = 0;
	}
	if( (pVideoInputFilter) ){ 	if(verbose)log_info(logFn, "SETUP: freeing Capture Source \n");
								(pVideoInputFilter)->Release();
								(pVideoInputFilter) = 0;
	}
	if( (pGrabberF) ){ 			if(verbose)log_info(logFn, "SETUP: freeing Grabber Filter  \n");
								(pGrabberF)->Release();
								(pGrabberF) = 0;
	}
	if( (pGrabber) ){ 			if(verbose)log_info(logFn, "SETUP: freeing Grabber  \n");
								(pGrabber)->Release();
								(pGrabber) = 0;
	}
	if( (pControl) ){ 			if(verbose)log_info(logFn, "SETUP: freeing Control   \n");
								(pControl)->Release();
								(pControl) = 0;
	}
	if( (pMediaEvent) ){ 		if(verbose)log_info(logFn, "SETUP: freeing Media Event  \n");
								(pMediaEvent)->Release();
								(pMediaEvent) = 0;
	}
	if( (streamConf) ){ 		if(verbose)log_info(logFn, "SETUP: freeing Stream  \n");
								(streamConf)->Release();
								(streamConf) = 0;
	}

	if( (pAmMediaType) ){ 		if(verbose)log_info(logFn, "SETUP: freeing Media Type  \n");
								MyDeleteMediaType(pAmMediaType);
	}

	if((pMediaEvent)){
			if(verbose)log_info(logFn, "SETUP: freeing Media Event  \n");
			(pMediaEvent)->Release();
			(pMediaEvent) = 0;
	}
	...
	if( (pCaptureGraph) ){ 		if(verbose)log_info(logFn, "SETUP: freeing Capture Graph \n");
								(pCaptureGraph)->Release();
								(pCaptureGraph) = 0;
	}
	if( (pGraph) ){ 			if(verbose)log_info(logFn, "SETUP: freeing Main Graph \n");
								(pGraph)->Release();
								(pGraph) = 0;
	}
	...
	if(verbose)log_info(logFn, "SETUP: Device %i disconnected and freed\n\n",myID);
}
...
// ----------------------------------------------------------------------
videoInput::videoInput(log_fn_t _logFn){
	//save log
	logFn = _logFn;
    ...
    for(int i=0; i<VI_MAX_CAMERAS; i++)  VDList[i] = new videoDevice(logFn);

    if(verbose)log_warn(logFn, "\n***** VIDEOINPUT LIBRARY - %2.04f - TFW07 *****\n\n",VI_VERSION);
...
void videoInput::setUseCallback(bool useCallback){
	if(callbackSetCount == 0){
		...
		log_err(logFn, "ERROR: setUseCallback can only be called before setup\n");
	}
}
...
bool videoInput::setFormat(int deviceNumber, int format){
	...
	if(format >= 0 && format < VI_NUM_FORMATS){
		...
		if(VDList[deviceNumber]->specificFormat){
			...
			if(FAILED(hr)){
				log_err(logFn, "SETUP: couldn't set requested format\n");
			}else{
				long lValue = 0;
...
//
// ----------------------------------------------------------------------
int videoInput::listDevices(bool silent, log_fn_t logFn){
    return _listDevices(silent, logFn, true);
}

int videoInput::listDevicesWithoutResolutionList(bool silent, log_fn_t logFn){
    return _listDevices(silent, logFn, false);
}

int videoInput::_listDevices(bool silent, log_fn_t logFn, bool cacheResolutionList){

    // Keep track of the number of devices we find.
    int deviceCounter = 0;

    //COM Library Intialization
	comInit(logFn);

    if(!silent)log_info(logFn, "\nVIDEOINPUT SPY MODE!\n\n");

    // Create a system device enumerator.
	ICreateDevEnum *pAllSystemDevicesEnum = NULL;

    // Get the system device enumerator. This gets all of the physical devices
    // connected to the PC.
	HRESULT hr = CoCreateInstance(CLSID_SystemDeviceEnum, NULL,
                                  CLSCTX_INPROC_SERVER, IID_ICreateDevEnum,
                                  reinterpret_cast<void**>(&pAllSystemDevicesEnum));

    if (FAILED(hr)) {
        log_err(logFn, "Error 0x%X - Could not retrieve list of all system devices.\n", hr);

        // Safely release anything that we've used up to this point,
        // uninitialize COM, and return 0, since we didn't find anything.
        SAFE_RELEASE(pAllSystemDevicesEnum);

        comUnInit();
        return 0;
    }

    // Create the video input device category enumerator.
    IEnumMoniker *pVideoDevicesEnum = NULL;

    // Create an enumerator for the video capture category.
    hr = pAllSystemDevicesEnum->CreateClassEnumerator(
                CLSID_VideoInputDeviceCategory,
                &pVideoDevicesEnum, 0);

    // Normally, we would use FAILED(hr) to test if the hr code came back with an error,
    // however, CoCreateInstance can also return S_FALSE, which is NOT an error (it's a positive
    // integer just like S_OK). It returns S_FALSE if the category specified by the first argument
    // to CoCreateInstance does not exist or is empty. When this happens, pVideoDevicesEnum will be NULL,
    // which means there are no physical devices attached to this machine. So we'll check if the hr code
    // is not S_OK for all the bad things that can happen.
    if (hr != S_OK)
    {
        if (hr == S_FALSE) {
            // We catch this as an error because this means that pVideoDevicesEnum came back NULL.
            log_info(logFn, "INFO - There are no video devices physically connected to this machine.\n");
        }
        else {
            log_err(logFn, "ERROR 0x%X - Could not retrieve list of all video devices.\n", hr);
        }

        // Safely release anything that we've used up to this point,
        // uninitialize COM, and return 0, since we didn't find anything.
        SAFE_RELEASE(pVideoDevicesEnum);
        SAFE_RELEASE(pAllSystemDevicesEnum);
    ...
    // Create a pointer to hold the capture device.
    // Monikers uniquely identify a COM object.
    IMoniker *pDeviceMoniker = NULL;

    // To retrieve a device, we loop through the video device enumerator.
    // The loop will stop when we fail to retrieve the next device.
    // NOTE: Do NOT use FAILED(hr) or SUCCEEDED(hr) here. Next() returns
    //       S_OK when the next device was retrieved, and S_FALSE
    //       otherwise. S_FALSE is a success code, meaning the function
    //       ran to completion BUT the function did not find the next
    //       device. So just check for S_OK here.
    while (pVideoDevicesEnum->Next(1, &pDeviceMoniker, NULL) == S_OK) {

        // Get the properties of the device.
        IPropertyBag *pDevicePropertyBag = NULL;

        hr = pDeviceMoniker->BindToStorage(0, 0, IID_IPropertyBag,
            (void**)(&pDevicePropertyBag));

        if (FAILED(hr)) {

            log_err(logFn, "Error 0x%X - Could not retrieve the properties of this device.\n", hr);

            // Release anything that we've used in the while loop up to this point.
            SAFE_RELEASE(pDevicePropertyBag);
            SAFE_RELEASE(pDeviceMoniker);

            // Skip this device, maybe the next one will work.
            continue;
        }
        ...
        VARIANT varName;
        VariantInit(&varName);
        hr = pDevicePropertyBag->Read(L"FriendlyName", &varName, 0);

        if (FAILED(hr)){
            log_err(logFn, "Error 0x%X - Failed to retrieve the friendly name of this device.\n", hr);
        ...
        // TODO: the -1 seems unneeded here, but it does guarantee
        // that we are null terminated if the name is maxLen or longer
        // characters.
        int maxLen = sizeof(deviceNames[0])/sizeof(deviceNames[0][0]) - 1;
        WideCharToMultiByte(CP_UTF8, 0, varName.bstrVal, -1,
                            deviceNames[deviceCounter], maxLen, 0, 0);

        if(!silent)log_info(logFn, "SETUP: %i) %s \n",deviceCounter, deviceNames[deviceCounter]);

        if (cacheResolutionList) {
            // While we're caching names, let's also cache other info about the device.
            hr = cacheDeviceSupportedResolutions(deviceCounter, &varName, pDeviceMoniker, logFn);

            if (hr != S_OK)
            {
                log_err(logFn, "Error 0x%X - Failed to cache supported resolutions for device: %i) %s.\n", hr, deviceCounter, deviceNames[deviceCounter]);
            }
        }
    ...
    if(!silent)log_info(logFn, "SETUP: %i Device(s) found\n\n", deviceCounter);
...
// ----------------------------------------------------------------------
// Our static function for getting num supported resolutions of deviceID.
// Must call listDevices first.
//
// ----------------------------------------------------------------------
char videoInput::numResolutionsPerDevice[VI_MAX_CAMERAS] = {0};

int videoInput::getNumSupportedResolutionsOfDevice(int deviceID, int *numSupportedResolutions, log_fn_t logFn){

	if ( ( deviceID >= VI_MAX_CAMERAS ) || ( deviceID < 0 ) ){
		return -1;
	}

    *numSupportedResolutions = numResolutionsPerDevice[deviceID];
}


// ----------------------------------------------------------------------
// Our static function for getting the resolution indexed by resPairIdx of deviceID.
// Must call listDevices first.
//
// ----------------------------------------------------------------------
int videoInput::deviceResolutions[VI_MAX_CAMERAS][VI_MAX_RESOLUTIONS][2] = {{{0}}};

int videoInput::getSupportedResolutionPairOfDevice(int deviceID, int resPairIdx, int *width, int *height, log_fn_t logFn){

	if ( ( deviceID >= VI_MAX_CAMERAS ) || ( deviceID < 0 ) ){
		return -1;
	}

    if ( ( resPairIdx >= VI_MAX_RESOLUTIONS ) || ( resPairIdx < 0 ) ){
		return -1;
	}

    *width = deviceResolutions[deviceID][resPairIdx][0];
    *height = deviceResolutions[deviceID][resPairIdx][1];
		...
		if(bCallback){
			...
			if(hr==S_OK){
				...
				if (numBytes == bufferSize){
					...
					log_warn(logFn, "ERROR: GetPixels() - bufferSizes do not match!\n");
				}
			}else{
				log_warn(logFn, "ERROR: GetPixels() - Unable to grab frame for device %i\n", id);
			}
		}
	}
...
bool videoInput::isFrameNew(int id){
	...
	if(freeze && VDList[id]->autoReconnect){
		log_warn(logFn, "ERROR: Device seems frozen - attempting to reconnect\n");
		if( !restartDevice(VDList[id]->myID) ){
			log_warn(logFn, "ERROR: Unable to reconnect to device\n");
		}else{
			log_warn(logFn, "SUCCESS: Able to reconnect to device\n");
		}
	}
...
bool videoInput::getVideoSettingFilter(int deviceID, long Property, long &min, long &max, long &SteppingDelta, long &currentValue, long &flags, long &defaultValue){
	...
	if (FAILED(hr)){
		log_err(logFn, "setVideoSetting - getDevice Error\n");
		return false;
	}
	...
	if(FAILED(hr)){
		log_err(logFn, "setVideoSetting - QueryInterface Error\n");
		if(VD->pVideoInputFilter)VD->pVideoInputFilter->Release();
		if(VD->pVideoInputFilter)VD->pVideoInputFilter = NULL;
		return false;
	}

	if (verbose) log_warn(logFn, "Setting video setting %ld.\n", Property);

	pAMVideoProcAmp->GetRange(Property, &min, &max, &SteppingDelta, &defaultValue, &flags);
	if (verbose) log_warn(logFn, "Range for video setting %ld: Min:%ld Max:%ld SteppingDelta:%ld Default:%ld Flags:%ld\n", Property, min, max, SteppingDelta, defaultValue, flags);
...
bool videoInput::setVideoSettingFilterPct(int deviceID, long Property, float pctValue, long Flags){
	...
	if( range == stepAmnt ){
		...
		log_err(logFn, "RASTER - pctValue is %f - value is %i - step is %i - mod is %i - rasterValue is %i\n", pctValue, value, stepAmnt, mod, rasterValue);
	}
...
bool videoInput::getVideoSettingCamera(int deviceID, long Property, long &min, long &max, long &SteppingDelta, long &currentValue, long &flags, long &defaultValue){
	...
	if (verbose) log_warn(logFn, "Getting video setting %ld.\n", Property);
...
void videoInput::stopDevice(int id){
	...
	{
		delete VDList[id];
		VDList[id] = new videoDevice(logFn);
	}
...
bool videoInput::comInit(log_fn_t logFn){
	HRESULT hr = (HRESULT)NULL;
	...
	if(comInitCount == 0 ){
		...
		if( hr == RPC_E_CHANGED_MODE){
			 log_warn(logFn, "SETUP - COM already setup - threaded VI might not be possible\n");
		}
	}
...
bool videoInput::setup(int deviceNumber){
    ...
    {
    	log_warn(logFn, "SETUP: device[%i] not found - you have %i devices available\n", deviceNumber, devicesFound);
    	if(devicesFound>=0) log_warn(logFn, "SETUP: this means that the last device you can use is device[%i] \n",  devicesFound-1);
    	return false;
    }

    if(VDList[deviceNumber]->readyToCapture)
    {
    	log_warn(logFn, "SETUP: can't setup, device %i is currently being used\n",VDList[deviceNumber]->myID);
    	return false;
    }
...
void videoInput::getMediaSubtypeAsString(GUID type, char * typeAsString){
	...
	else if(type == MEDIASUBTYPE_RGB565)strcpy(tmpStr, "RGB565");
	else if(type == MEDIASUBTYPE_YUY2) 	strcpy(tmpStr, "YUY2");
	else if(type == MEDIASUBTYPE_YVYU) 	strcpy(tmpStr, "YVYU");
	else if(type == MEDIASUBTYPE_YUYV) 	strcpy(tmpStr, "YUYV");
	else if(type == MEDIASUBTYPE_IYUV) 	strcpy(tmpStr, "IYUV");
	else if(type == MEDIASUBTYPE_UYVY)  strcpy(tmpStr, "UYVY");
	else if(type == MEDIASUBTYPE_YV12)  strcpy(tmpStr, "YV12");
	else if(type == MEDIASUBTYPE_YVU9)  strcpy(tmpStr, "YVU9");
	else if(type == MEDIASUBTYPE_Y411) 	strcpy(tmpStr, "Y411");
	else if(type == MEDIASUBTYPE_Y41P) 	strcpy(tmpStr, "Y41P");
	else if(type == MEDIASUBTYPE_Y211)  strcpy(tmpStr, "Y211");
	else if(type == MEDIASUBTYPE_AYUV) 	strcpy(tmpStr, "AYUV");
	else if(type == MEDIASUBTYPE_Y800) 	strcpy(tmpStr, "Y800");
	else if(type == MEDIASUBTYPE_Y8)   	strcpy(tmpStr, "Y8");
	else if(type == MEDIASUBTYPE_GREY) 	strcpy(tmpStr, "GREY");
	else strcpy(tmpStr, "OTHER");
...
int videoInput::start(int deviceID, videoDevice *VD){
    ...
    if(verbose)log_info(logFn, "SETUP: Setting up device %i\n",deviceID);
    ...
    {
        log_warn(logFn, "ERROR 0x%X - Could not create the Filter Graph Manager\n", hr);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
		log_warn(logFn, "ERROR 0x%X - Could not add the graph builder!\n", hr);
	    stopDevice(deviceID);
        return hr;
	}
	...
	if (FAILED(hr))
    {
		log_warn(logFn, "ERROR 0x%X - Could not set filtergraph\n", hr);
	    stopDevice(deviceID);
        return hr;
	}
    ...
    if (FAILED(hr))
    {
        log_warn(logFn, "ERROR 0x%X - Could not create the Media Control object\n", hr);
       	stopDevice(deviceID);
        return hr;
    }
	...
	if (SUCCEEDED(hr)){
		if(verbose)log_info(logFn, "SETUP: %s\n", VD->nDeviceName);
		hr = VD->pGraph->AddFilter(VD->pVideoInputFilter, VD->wDeviceName);

	    if (FAILED(hr)){
	        log_err(logFn, "ERROR 0x%X - Could not add specified video input filter to the graph manager.\n", hr);
	        stopDevice(deviceID);
	        return hr;
	    }

	}else{
        log_warn(logFn, "ERROR 0x%X - Could not find specified video device\n", hr);
        stopDevice(deviceID);
        return hr;
	}
	...
	if(FAILED(hr)){
		log_warn(logFn, "SETUP 0x%X - Couldn't find preview pin.\n", hr);
	}else{
	...
	if(VD->useCrossbar)
	{
		if(verbose)log_info(logFn, "SETUP: Checking crossbar\n");
	...
	if(FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X -  Couldn't config the stream!\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if(FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X -  Couldn't getFormat for pAmMediaType!\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if(customSize){
		if(verbose) log_info(logFn, "SETUP: Default Format is set to %i by %i \n", currentWidth, currentHeight);
		...
		for(int i = 0; i < VI_NUM_TYPES; i++){
			...
			if(verbose)log_info(logFn, "SETUP: trying format %s @ %i by %i\n", guidStr, VD->tryWidth, VD->tryHeight);
		...
		if( foundSize == false ){
			if(verbose) log_info(logFn, "SETUP: couldn't find requested size - searching for closest matching size\n");
			...
			if( closestWidth != -1 && closestHeight != -1){
                ...
                // Sometimes while findClosestSizeAndSubtype will return the proper size, it will return
                // an invalid media type.  If this is the case we need to iterate over until we successfully
                // set the size and subtype.  It doesn't hurt to do this in all cases, so that's what we do.
                for (int i = 0; i < VI_NUM_TYPES; i++) {
                    getMediaSubtypeAsString(mediaSubtypes[i], guidStr);

                    if(verbose)log_info(logFn, "SETUP: trying format %s @ with new size %i by %i\n",
                                        guidStr, closestWidth, closestHeight);
                    if (setSizeAndSubtype(VD, closestWidth, closestHeight, mediaSubtypes[i])) {
	...
	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not Create Sample Grabber - CoCreateInstance()\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not add Sample Grabber - AddFilter()\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not query SampleGrabber\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if(bCallback){
		...
		if (FAILED(hr)){
			log_warn(logFn, "ERROR 0x%X - problem setting callback\n", hr);
			stopDevice(deviceID);
			return hr;
		}else{
			if(verbose)log_info(logFn, "SETUP: Capture callback set\n");
		}
	}
	...
	if(VD->streamConf){
		...
		log_warn(logFn, "ERROR: connecting device - prehaps it is already being used?\n");
		stopDevice(deviceID);
		return S_FALSE;
	}
	...
	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not create filter - NullRenderer\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not add filter - NullRenderer\n", hr);
		stopDevice(deviceID);
		return hr;
	}
	...
	// We want to try to grab a preview pin, even if the webcam doesn't have one
	// and is setup to use its capture pin, because it's supposedly faster. If
	// we ask "RenderStream" to connect the graphs together using the preview
	// pin, but it doesn't exist for the filter we give it (which happens most
	// of the time for webcams), then it will automatically add a "SmartTee"
	// filter that splits the capture pin of the webcam into a preview pin and
	// capture pin, and connects the graph using the preview pin. If, for some
	// reason, this doesn't work, we will try to use the capture pin that is
	// already available to us from the webcam.
	hr = VD->pCaptureGraph->RenderStream(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, VD->pVideoInputFilter, VD->pGrabberF, VD->pDestFilter);

	if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not connect preview pin using SmartTee filter - RenderStream()\n", hr);

	    // Try to connect with the capture pin before we give up...
	    hr = VD->pCaptureGraph->RenderStream(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, VD->pVideoInputFilter, VD->pGrabberF, VD->pDestFilter);

        if (FAILED(hr)){
            log_warn(logFn, "ERROR 0x%X - Could not connect to capture pin - RenderStream()\n", hr);
		    stopDevice(deviceID);
		    return hr;
		}
	}
	...
	//EXP - lets try setting the sync source to null - and make it run as fast as possible
	{
		...
		if (FAILED(hr)){
			log_warn(logFn, "ERROR 0x%X - Could not get IID_IMediaFilter interface\n", hr);
		}else{
	...
	if (FAILED(hr)){
		 log_warn(logFn, "ERROR 0x%X - Could not start graph\n", hr);
		 stopDevice(deviceID);
		 return hr;
	}
	...
	if(verbose)log_info(logFn, "SETUP: Device is setup and ready to capture.\n\n");
...
// ----------------------------------------------------------------------
// Caches the given deviceID's supported resolutions.
// This should be called by listDevices().
// ----------------------------------------------------------------------

int videoInput::cacheDeviceSupportedResolutions(int deviceID, VARIANT *varName, IMoniker *pMoniker, log_fn_t logFn){

	if ( ( deviceID >= VI_MAX_CAMERAS ) || ( deviceID < 0 ) ){
		return E_FAIL;
	}

	numResolutionsPerDevice[deviceID] = 0;
    ...
    if(verbose)log_info(logFn, "CACHE - Caching supported resolutions for device %i\n", deviceID);
	...
	//GET THE VIDEO DEVICE AND ADD TO GRAPH//
    if (varName && pMoniker) {
	    ...
	    hr = pMoniker->BindToObject(NULL, NULL, IID_IBaseFilter, (void**)&pVideoInputFilter);
    }
    else {
        // didn't find the video device: error
        hr = VFW_E_NOT_FOUND;
    }
    ...
    if (pGraph && pVideoInputFilter && wDeviceName) {
    ...
    if (FAILED(hr)){
		log_warn(logFn, "ERROR 0x%X - Could not add filter - %s\n", hr, nDeviceName);
        SAFE_RELEASE(pVideoInputFilter);
        SAFE_RELEASE(pGraph);
	    SAFE_RELEASE(pBuilder);
		return hr;
	...
	//LOOK FOR PREVIEW PIN.
    if (pBuilder && pVideoInputFilter) {
        hr = pBuilder->FindInterface(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, pVideoInputFilter, IID_IAMStreamConfig, (void **)&pStreamConf);
    }
    else {
        hr = E_UNEXPECTED;
	...
	if(FAILED(hr)){
        // If we're here, it's because most webcams don't have a preview pin;
        // they only have capture pins. We'll search for a capture pin,
        // but if we can't find that either, we'll log this as an error.
        // For analog devices, they don't have preview pins, nor
        // capture pins; however, their version of the preview pin is compatible
        // to this search, and should always succeed.
        HRESULT first_hr = hr;
        if (pBuilder && pVideoInputFilter) {
            hr = pBuilder->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, pVideoInputFilter, IID_IAMStreamConfig, (void **)&pStreamConf);
        }
        else {
            hr = E_UNEXPECTED;
        }
        if(FAILED(hr)){
            // If we get here, something went horribly wrong. Log it.
            log_warn(logFn, "ERROR 0x%X, 0x%X - Could not find the Preview Pin "
                     "nor the Capture Pin of this device for stream configuration!"
                     "The first error code is related to the Preview Pin."
                     "The second error code is related to the Capture Pin.\n", first_hr, hr);
            SAFE_RELEASE(pStreamConf);
            SAFE_RELEASE(pVideoInputFilter);
            SAFE_RELEASE(pGraph);
            SAFE_RELEASE(pBuilder);
    ...
    // This is the number of formats the camera has.
	int iCount = 0;
	int iSize = 0;
    if (pStreamConf) {
    ...
    if (FAILED(hr)) {
        // This shouldn't fail, since streamConf was created successfully.
        log_warn(logFn, "ERROR 0x%X - Could not get the device capabilities!", hr);
        SAFE_RELEASE(pStreamConf);
        SAFE_RELEASE(pVideoInputFilter);
        SAFE_RELEASE(pGraph);
        SAFE_RELEASE(pBuilder);
    ...
    // GET SUPPORTED RESOLUTIONS FROM THE DEVICE.
	if (iSize == sizeof(VIDEO_STREAM_CONFIG_CAPS)) {
		// Use the video capabilities structure.
		for (int iFormat = 0; (iFormat < iCount) && (numResolutionsPerDevice[deviceID] < VI_MAX_RESOLUTIONS); iFormat++) {
			VIDEO_STREAM_CONFIG_CAPS scc;
            VIDEOINFOHEADER *pVih;

            if (pStreamConf) {
                hr = pStreamConf->GetStreamCaps(iFormat, &pAmMediaType, (BYTE*)&scc);
            }
            else {
                hr = E_UNEXPECTED;
            }
            if ( FAILED(hr) || (hr == S_FALSE) ) {
                // If (hr == S_FALSE) then,
                // this means the index (iFormat) is too high. For some reason,
                // our index is now out of bounds.
                // If FAILED(hr) then,
                // we're here if we passed a NULL pointer to GetStreamCaps,
                // not enough memory, we passed in an invalid index, or the
                // input pin is not connected.
                log_warn(logFn, "ERROR 0x%X - Couldn't get the device format.", hr);

                // Free the pAmMediaType.
                SAFE_RELEASE_AM_MEDIA_TYPE(pAmMediaType);

                // Break out of the loop.
                break;
			}

            if (pAmMediaType->cbFormat < sizeof(VIDEOINFOHEADER)) {
                // It's possible for pbFormat to be NULL. So if cbFormat
                // is less than the size of a VIDEOINFOHEADER, we can't
                // use pbFormat. Log this as an error, but continue the loop.
                log_warn(logFn, "ERROR - Couldn't get the device format.");

                // Free the pAmMediaType.
                SAFE_RELEASE_AM_MEDIA_TYPE(pAmMediaType);

                // It's safe to continue the loop.
                continue;
            }
            // Examine the format.
            pVih = (VIDEOINFOHEADER*)pAmMediaType->pbFormat;

            if (pVih) {
                int duplicateExists = 0;

                // Check for duplicates.
                for (int i = 0; i < numResolutionsPerDevice[deviceID]; i++) {
                    if ( (deviceResolutions[deviceID][i][0] == pVih->bmiHeader.biWidth) &&
                         (deviceResolutions[deviceID][i][1] == pVih->bmiHeader.biHeight) ) {
                        duplicateExists = 1;
                        break;
                    }
                }

                // If not currently in the list, add resolution pair to resolution list.
                if (!duplicateExists) {
                    deviceResolutions[deviceID][numResolutionsPerDevice[deviceID]][0] = pVih->bmiHeader.biWidth;
                    deviceResolutions[deviceID][numResolutionsPerDevice[deviceID]][1] = pVih->bmiHeader.biHeight;
                    numResolutionsPerDevice[deviceID]++;
                }
            }
            // Free the pAmMediaType.
            SAFE_RELEASE_AM_MEDIA_TYPE(pAmMediaType);
...
HRESULT videoInput::routeCrossbar(ICaptureGraphBuilder2 **ppBuild, IBaseFilter **pVidInFilter, int conType, GUID captureMode){
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if(verbose)
        {
            log_info(logFn, "SETUP: You are not a webcam! Setting Crossbar\n");
            DisplayCrossbarInfo(logFn, pXBar1);
        }
        pXBar1->Release();
        ...
        while( pIndex < lInpin)
        {
            ...
            if( pType == conType){
                    if(verbose)log_info(logFn, "SETUP: Found Physical Interface");

                    switch(conType){

                        case PhysConn_Video_Composite:
                            if(verbose)log_info(logFn, "\tComposite\n");
                            break;
                        case PhysConn_Video_SVideo:
                            if(verbose)log_info(logFn, "\tS-Video\n");
                            break;
                        case PhysConn_Video_Tuner:
                            if(verbose)log_info(logFn, "\tTuner\n");
                            break;
                        case PhysConn_Video_USB:
                             if(verbose)log_info(logFn, "\tUSB\n");
                            break;
                        case PhysConn_Video_1394:
                            if(verbose)log_info(logFn, "\tFirewire\n");
                            break;
                    }
        ...
        if(foundDevice){
            ...
            if(verbose)log_info(logFn, "SETUP: Didn't find specified Physical Connection type. Using Default. \n");
        }
        ...
        if(verbose)log_info(logFn, "SETUP: You are a webcam or snazzy firewire cam! No Crossbar needed\n");
        return hr;
    }
